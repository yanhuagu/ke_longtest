...
@ddt.ddt
class queryTest_learnkylin(unittest.TestCase):
    ...
    def tearDownClass(self):
        ...
        queryTest_learnkylin.cur.close()
        queryTest_learnkylin.conn.close()
        pass
...
##比较运算符
    ...
    def testCubeQuery_learnkylin(self,payload):
        """
            testQuery
        """
        query_url = queryTest_learnkylin.base_url + "/query"
        starttime = datetime.now()
        response = requests.request("POST", query_url, data=payload, headers=queryTest_learnkylin.headers)
        ...
        '''
        sql = sql.format(namepro="queryTest_learnkylin",request1=str(response.url),respons1=str(response.text),timestart = starttime,timeend = timeend,D_time = dtime,status = str(response.status_code))
        queryTest_learnkylin.cur.execute(sql)
        queryTest_learnkylin.conn.commit()
        self.assertEqual(
...
if __name__ == '__main__':
    ...
    test_suite.addTest(unittest.makeSuite(queryTest_learnkylin))
    run(test_suite)
