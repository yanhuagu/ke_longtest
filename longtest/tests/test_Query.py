...
class queryTest(unittest.TestCase):
    ...
    @ ddt.data(
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  l_returnflag,  l_linestatus,  sum(l_quantity) as sum_qty,  sum(l_extendedprice) as sum_base_price,  sum(l_extendedprice * (1 - l_discount)) as sum_disc_price,  sum(l_extendedprice * (1 - l_discount) * (1 + l_tax)) as sum_charge,  avg(l_quantity) as avg_qty,  avg(l_extendedprice) as avg_price,  avg(l_discount) as avg_disc, count(*) as count_order from lineitem where l_shipdate <= '1998-09-16' group by l_returnflag, l_linestatus order by l_returnflag,l_linestatus;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q1
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  p_partkey as min_p_partkey,  min(ps_supplycost) as min_ps_supplycost from  part,  partsupp,  supplier,  nation,  region where  p_partkey = ps_partkey  and s_suppkey = ps_suppkey  and s_nationkey = n_nationkey  and n_regionkey = r_regionkey  and r_name = 'EUROPE' group by  p_partkey;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q2
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"  select  s_acctbal,  s_name,  n_name,  p_partkey,  p_mfgr,  s_address,  s_phone,  s_comment from  part,  supplier,  partsupp,  nation,  region,  q2_min_ps_supplycost where  p_partkey = ps_partkey  and s_suppkey = ps_suppkey  and p_size = 37  and p_type like '%COPPER'  and s_nationkey = n_nationkey  and n_regionkey = r_regionkey  and r_name = 'EUROPE'  and ps_supplycost = min_ps_supplycost  and p_partkey = min_p_partkey order by  s_acctbal desc,  n_name,  s_name,  p_partkey limit 100;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q2
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  l_orderkey,  sum(l_extendedprice * (1 - l_discount)) as revenue,  o_orderdate,  o_shippriority from  customer,  orders,  lineitem where  c_mktsegment = 'BUILDING'  and c_custkey = o_custkey  and l_orderkey = o_orderkey  and o_orderdate < '1995-03-22'  and l_shipdate > '1995-03-22' group by  l_orderkey,  o_orderdate,  o_shippriority order by  revenue desc,  o_orderdate limit 10; ","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q3
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    o_orderpriority,    count(*) as order_count  from    orders as o  where    o_orderdate >= '1996-05-01'    and o_orderdate < '1996-08-01'    and exists (      select        *      from        lineitem      where        l_orderkey = o.o_orderkey        and l_commitdate < l_receiptdate    )  group by    o_orderpriority  order by    o_orderpriority;  ","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q4
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    _name,    sum(l_extendedprice * (1 - l_discount)) as revenue  from    customer,    orders,    lineitem,    supplier,    nation,    region  where    c_custkey = o_custkey    and l_orderkey = o_orderkey    and l_suppkey = s_suppkey    and c_nationkey = s_nationkey    and s_nationkey = n_nationkey    and n_regionkey = r_regionkey    and r_name = 'AFRICA'    and o_orderdate >= '1993-01-01'    and o_orderdate < '1994-01-01'  group by    n_name  order by    revenue desc;  ","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q5
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    tsum(l_extendedprice * l_discount) as revenue  from    lineitem  where    l_shipdate >= '1993-01-01'    and l_shipdate < '1994-01-01'    and l_discount between 0.06 - 0.01 and 0.06 + 0.01    and l_quantity < 25;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q6
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    supp_nation,    cust_nation,    l_year,    sum(volume) as revenue  from    (      select        n1.n_name as supp_nation,        n2.n_name as cust_nation,        year(l_shipdate) as l_year,        l_extendedprice * (1 - l_discount) as volume      from        supplier,        lineitem,        orders,        customer,        nation n1,        nation n2      where        s_suppkey = l_suppkey        and o_orderkey = l_orderkey        and c_custkey = o_custkey        and s_nationkey = n1.n_nationkey        and c_nationkey = n2.n_nationkey        and (          (n1.n_name = 'KENYA' and n2.n_name = 'PERU')          or (n1.n_name = 'PERU' and n2.n_name = 'KENYA')        )        and l_shipdate between '1995-01-01' and '1996-12-31'    ) as shipping  group by    supp_nation,    cust_nation,    l_year  order by    supp_nation,    cust_nation,    l_year;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q7
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    o_year,    sum(case      when nation = 'PERU' then volume      else 0    end) / sum(volume) as mkt_share  from    (      select        year(o_orderdate) as o_year,        l_extendedprice * (1 - l_discount) as volume,        n2.n_name as nation      from        part,        supplier,        lineitem,        orders,        customer,        nation n1,        nation n2,        region      where        p_partkey = l_partkey        and s_suppkey = l_suppkey        and l_orderkey = o_orderkey        and o_custkey = c_custkey        and c_nationkey = n1.n_nationkey        and n1.n_regionkey = r_regionkey        and r_name = 'AMERICA'        and s_nationkey = n2.n_nationkey        and o_orderdate between '1995-01-01' and '1996-12-31'        and p_type = 'ECONOMY BURNISHED NICKEL'    ) as all_nations  group by    o_year  order by    o_year;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q8
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    nation,    o_year,    sum(amount) as sum_profit  from    (      select        n_name as nation,        year(o_orderdate) as o_year,        l_extendedprice * (1 - l_discount) - ps_supplycost * l_quantity as amount      from        part,        supplier,        lineitem,        partsupp,        orders,        nation      where        s_suppkey = l_suppkey        and ps_suppkey = l_suppkey        and ps_partkey = l_partkey        and p_partkey = l_partkey        and o_orderkey = l_orderkey        and s_nationkey = n_nationkey        and p_name like '%plum%'    ) as profit  group by    nation,    o_year  order by    nation,    o_year desc;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q9
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select    c_custkey,    c_name,    sum(l_extendedprice * (1 - l_discount)) as revenue,    c_acctbal,    n_name,    c_address,    c_phone,    c_comment  from    customer,    orders,    lineitem,    nation  where    c_custkey = o_custkey    and l_orderkey = o_orderkey    and o_orderdate >= '1993-07-01'    and o_orderdate < '1993-10-01'    and l_returnflag = 'R'    and c_nationkey = n_nationkey  group by    c_custkey,    c_name,    c_acctbal,    c_phone,    n_name,    c_address,    c_comment  order by    revenue desc  limit 20;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q10
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  ps_partkey,  sum(ps_supplycost * ps_availqty) as part_value from  partsupp,  supplier,  nation where  ps_suppkey = s_suppkey  and s_nationkey = n_nationkey  and n_name = 'GERMANY' group by ps_partkey; ","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q11
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  ps_partkey, part_value as value from (  select   ps_partkey,   part_value,   total_value  from   q11_part_tmp_cached join q11_sum_tmp_cached ) a where  part_value > total_value * 0.0001 order by  value desc;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q11
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  l_shipmode,  sum(case   when o_orderpriority = '1-URGENT'    or o_orderpriority = '2-HIGH'    then 1   else 0  end) as high_line_count,  sum(case   when o_orderpriority <> '1-URGENT'    and o_orderpriority <> '2-HIGH'    then 1   else 0  end) as low_line_count from  orders,  lineitem where  o_orderkey = l_orderkey  and l_shipmode in ('REG AIR', 'MAIL')  and l_commitdate < l_receiptdate  and l_shipdate < l_commitdate  and l_receiptdate >= '1995-01-01'  and l_receiptdate < '1996-01-01' group by  l_shipmode order by  l_shipmode;","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q12
        ['''{"acceptPartial":true,"limit":50000,"offset":0,"project":"aaa","sql":"select  100.00 * sum(case   when p_type like 'PROMO%'    then l_extendedprice * (1 - l_discount)   else 0  end) / sum(l_extendedprice * (1 - l_discount)) as promo_revenue from  lineitem,  part where  l_partkey = p_partkey  and l_shipdate >= '1995-08-01'  and l_shipdate < '1995-09-01';","backdoorToggles":{"DEBUG_TOGGLE_HTRACE_ENABLED":false}}'''],#Q14
